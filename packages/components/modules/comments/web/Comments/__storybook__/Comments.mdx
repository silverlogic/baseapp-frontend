...
- **Provider Note**: `Comments` includes the `CommentReplyProvider` internally. If you already wrap a page/section with the provider (so you can call `useCommentReply` higher up), use `BaseComments` instead to avoid double providers.
...
```javascript
import { Comments } from '@baseapp-frontend/design-system/web'
