...
import { IManagementComponent } from './types'

const ManagementComponent: FC<IManagementComponent> = ({
  subscriptionId,
  customerId,
  handleSetupSuccess,
}) => {
  ...
  const { data: paymentMethods, isLoading: isLoadingMethods } = useGetPaymentMethod(customerId)
  ...
  const { mutateAsync: updateSubscription } = useUpdateSubscription(
    ...
    refetchSubscription,
    {
      onSuccess: (response, variables) => {
        setSelectedPaymentMethodId(variables.defaultPaymentMethod || '')
  ...
  useEffect(() => {
    if (subscription?.defaultPaymentMethod && paymentMethods && paymentMethods?.length > 0) {
  ...
  }, [subscription?.defaultPaymentMethod, paymentMethods])
