...
import type { UseAllAuthLoginEmailCodeConfirm } from './types'

const useAllAuthLoginEmailCodeConfirm = ({
  formOptions = {},
  mutationOptions = {},
  accessKeyName = ACCESS_KEY_NAME,
  refreshKeyName = REFRESH_KEY_NAME,
  enableFormApiErrors = true,
}: UseAllAuthLoginEmailCodeConfirm) => {
  /*
  ...
  const mutation = useMutation({
    mutationFn: (data: AllAuthTypes.ConfirmLoginEmailCodeRequest) =>
      AllAuthApi.confirmLoginEmailCode(data),
  ...
  return {
    form: {
      ...
      handleSubmit: form.handleSubmit(async (values) => {
        await mutation.mutateAsync(values as AllAuthTypes.ConfirmLoginEmailCodeRequest)
      }) as any,
    },
    mutation,
  }
...
export default useAllAuthLoginEmailCodeConfirm
